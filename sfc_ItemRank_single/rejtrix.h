...
T findsigma(const Matrix &A, const Vector &b, I ur, 
            const Matrix &U, const Diagonal &s, const Matrix &V)
  {
    ...
    /*
    //older but exactly equivalent method that seemed clearer but isn't...
    ...
    */
    ...
    Vector Ub=transpose(U) * b;
    return (Ub.range(ur,mm-1)).rms();
  }
...
//old style sensitivity tracking to detect large instability
void splits2(int m, const Diagonal &s, const Vector &beta, int &rank)
{
  //default answer
  rank=0;
  if (m<1) return;        //no matrix
  if (s[0]<=0.0) return;  //matrix is zero

  //initialize
  double sensitivity = absf(beta[0]/s[0]);
  double small = sensitivity;
  double local = sensitivity;
  rank = 1;

  //look for sensitivity explosion
  
  for (int i=1; i<m; i++)  //start with 2nd row
    {
      if (s[i]<=0.0) break;
      sensitivity = absf(beta[i]/s[i]);

      //detect noise domination and perhaps split!
      if ( sensitivity > 15.0*small && sensitivity > local) break;

      //track small
      if (sensitivity < small)   small   = small   + 0.40*(sensitivity-small);  //2013
      else                       small   = small   + 0.02*(sensitivity-small);
...
Vector autoregusv(const Matrix &A, const Vector &b, I &ur,  T &sigma, T &lambda,
                  const Matrix &U, const Diagonal &s, const Matrix &V)
  {
    ...
    if (k<=0) return Vector(n); //zero system
    ...
    //if sensitivity tracking produces a later good urank, use it
    I ur2;
    splits2(k,s,beta,ur2);
    //cout << "urma, urse, k, p=" << ur << " " << ur2 << " " << k << " " << p << endl;
    if (ur2<k & ur2>ur) ur=ur2; 
...
Vector solvethreeway(const Matrix &AA, const Vector &bb, T lambda,
                     ...
                     const Matrix &GG, const Vector &hh)
  {
    ...
    for (int kk=0; kk<mg+1; kk++) 
      {
        ...
        //debug version 2.13
        //cout << "Loop: " << kk+1 << endl;
        //cout << "rankc: " << rankc << " AA.dmin: " << AA.dmin() << endl;
        //cout << "RMS of A2:" << A2.rms_as_vector() << endl;
        //cout << "RMS of A "  << A.rms_as_vector() << endl;
        //cout << "collapsed: " << collapsed(A,A2) << endl;
        //cout << "x:" << x << endl;
        //cout << "next inequality: " << r << endl;
        //prompt();
    ...
    //clean up roundoff...
    T small = minf(1000.0*Matrix::roundoff() , 0.0000000001);    
    T eps = small * x.maxabs();
    for (int i=0; i<n; i++) if (absf(x[i])<eps) x[i]=0.0;
...
Vector solvethreewayDIS4(const Matrix &AA, const Vector &bb, 
                   ...
                   const Matrix &GG, const Vector &hh)
  {
    checkdim(AA,bb,EE,ff,GG,hh,"solvethreewayDIS4");
    I m=AA.dim1();  
    I n=AA.dim2(); 
    if (m + EE.dim1() + GG.dim1() < 1) { Vector x(n); return x; } 
...
Vector solvethreewayTLS(const Matrix &AA, const Vector &bb,
  ...
  {
    checkdim(AA,bb,EE,ff,GG,hh,"solvethreewayTLS");
    I m=AA.dim1();  
    I n=AA.dim2(); 
    if (m + EE.dim1() + GG.dim1() < 1) { Vector x(n); return x; } 
